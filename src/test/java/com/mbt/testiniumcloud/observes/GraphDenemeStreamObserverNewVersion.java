...
public class GraphDenemeStreamObserverNewVersion implements Observer {

    private static final Logger logger = LoggerFactory.getLogger(GraphDenemeStreamObserverNewVersion.class);
    ...
    public GraphDenemeStreamObserverNewVersion(Class startClass, List<Context> contextList, boolean isEdgeTextVisibility, boolean alwaysOnTop) {

        isStartElement = true;
        styleSheet1 = String.format(stylesheet, isEdgeTextVisibility ? "normal" : "hidden");
                ...
                if (lastElement instanceof Vertex.RuntimeVertex) {
                    if(((Vertex.RuntimeVertex) lastElement).hasSharedState()){
                        ...
                        if (isStartElement && ((Vertex.RuntimeVertex) lastElement).getName().equals(startElementName)) {
                            ...
                            node.setAttribute("ui.color",0.40);
                            isStartElement = false;
                        }else {
                            Node node = multiGraph.getNode(getId(lastContext, lastElement));
                            node.addAttribute("ui.class", "success");
                            node.setAttribute("ui.color", 0.30);
                        }
                    }
                } else {
                    org.graphstream.graph.Edge graphEdge = multiGraph.getEdge(getId(lastContext, lastElement));
                    graphEdge.addAttribute("ui.class", "success");
                    graphEdge.setAttribute("ui.color", 0.30);
                }
            }

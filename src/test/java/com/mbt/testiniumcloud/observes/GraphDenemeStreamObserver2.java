...
public class GraphDenemeStreamObserver2 implements Observer {

    private static final Logger logger = LoggerFactory.getLogger(GraphDenemeStreamObserver2.class);
    ...
    public GraphDenemeStreamObserver2(Class startClass, List<Context> contextList) {

        isStartElement = true;
        System.setProperty("org.graphstream.ui.renderer", "org.graphstream.ui.j2dviewer.J2DGraphRenderer");
        frame = new JFrame("GRAPH STREAM VERSION 2.0");
                ...
                if (lastElement instanceof Vertex.RuntimeVertex) {
                    if(((Vertex.RuntimeVertex) lastElement).hasSharedState()){
                        ...
                        if (isStartElement && ((Vertex.RuntimeVertex) lastElement).getName().equals(startElementName)) {
                            ...
                            node.setAttribute("ui.color",0.4);
    ...
    private static String createVertex(Model.RuntimeModel model, MultiGraph graph, Vertex.RuntimeVertex runtimeVertex){
        ...
        if(node==null){
            ...
            node.setAttribute("x",(1.3d) * Double.parseDouble(map.get("x").toString()));
            node.setAttribute("y",(-1.3d) * Double.parseDouble(map.get("y").toString()));
        }
        return id;
    }
